    ...
    return (
        <Stack className={styles.container} tokens={{ childrenGap: 10 }}>
            <Dropdown
                ...
                options={[
                    { key: "hybrid", text: "ベクター + テキスト (ハイブリッド)", selected: retrievalMode == RetrievalMode.Hybrid, data: RetrievalMode.Hybrid },
                    { key: "vectors", text: "ベクター", selected: retrievalMode == RetrievalMode.Vectors, data: RetrievalMode.Vectors },
                    { key: "text", text: "テキスト", selected: retrievalMode == RetrievalMode.Text, data: RetrievalMode.Text }
                ]}
                required
            ...
            {showImageOptions && [RetrievalMode.Vectors, RetrievalMode.Hybrid].includes(retrievalMode) && (
                <Dropdown
                    ...
                    label="埋め込み (マルチクエリベクター検索)"
                    options={[
                        { key: VectorFieldOptions.Embedding, text: "テキスト埋め込み", selected: vectorFieldOption === VectorFieldOptions.Embedding },
                        { key: VectorFieldOptions.ImageEmbedding, text: "イメージ埋め込み", selected: vectorFieldOption === VectorFieldOptions.ImageEmbedding },
                        { key: VectorFieldOptions.Both, text: "テキストとイメージ埋め込み", selected: vectorFieldOption === VectorFieldOptions.Both }
                    ]}
